...
let hitPopTimeout = null;
  ...
  function update(dt) {
    ...
    try { if (particles.length > 600) particles.splice(0, particles.length - 600); } catch (e) { }
    ...
    try {
      for (let i=powerups.length-1;i>=0;i--) {
        ...
        try {
          ...
          if (Math.sqrt(dx*dx + dy*dy) < 140) { // slightly increased pickup radius to make power-ups easier to collect (now 140)

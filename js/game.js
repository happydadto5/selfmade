  ...
  if (typeof window !== 'undefined') {
    try {
      const showTouchGuide = () => {
        touchGuideExpires = Date.now() + 12000;
...
if (overlay) {
  ...
  const AUTO_PAUSE_DEBOUNCE = 450; // ms used for blur/visibility auto-pause debounce (small increase to reduce accidental pauses)
  let paused = false;

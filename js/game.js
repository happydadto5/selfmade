(() => {
  ...
  function refreshVersionHUD() {
    try {
      ...
      if (versionEl) {
        try { versionEl.textContent = 'v' + version + ' — High: ' + displayHigh + ((typeof autoPauseEnabled !== 'undefined' && autoPauseEnabled) ? ' — Auto-pause: On' : ' — Auto-pause: Off') + ' • Level: ' + (typeof levelNumber !== 'undefined' ? levelNumber : 1) + ' • Press P to pause'; } catch (e) { /* ignore DOM errors */ }
      }
          ...
          try {
            ...
            if (typeof player !== 'undefined' && now < (player.shieldUntil || 0)) {
              ...
              try {
                ...
                try { shieldSpan.title = 'Shield: ' + (sec ? sec + 's' : '') + (charges ? (' • ' + charges + ' charges') : '') + (charges ? ' • Press B to use' : ''); } catch(e) {}
                try { shieldSpan.setAttribute('aria-label', 'Shield: ' + (sec ? sec + ' seconds' : '') + (charges ? (', ' + charges + ' charges') : '') + (charges ? ', press B to use' : '')); } catch(e) {}
    ...
    bindTouchControl(touchShieldBtn, () => {
      try {
        if (player && typeof player.shieldCharges === 'number' && player.shieldCharges > 0) {
          ...
          player.shieldUntil = Math.max(Date.now(), player.shieldUntil || 0) + 10000; // +10s
  ...
  window.addEventListener('keydown', e => {
    if ((e.key === 'u' || e.key === 'U' || e.key === 'b' || e.key === 'B') && !gameOver) {
      try {
        if (player && typeof player.shieldCharges === 'number' && player.shieldCharges > 0) {
          const now = Date.now();
          // grant ~10s of shield and decrement charges
  ...
  try {
    ...
    for (let i=0;i<count;i++) {
      ...
      if (isZig) {
        ...
        enemies.push({x:ex,y:ey,w:28,h:24,vy:speed*0.9, vx:0, hp:1, maxHp:1, type:'hopper', hopTimer: 360 + Math.random()*420, hopStrength: 3.6 + Math.random()*2.4, t: Math.random()*1000});
      } else if (isSprout) {
            ...
            if (pu.type === 'rapid') {
              ...
              try {
                ...
                if (player && now < (player.shieldUntil || 0)) {
                  ...
                  // new shield
                  player.shieldUntil = Date.now() + 15000; // 15 seconds (tuned for pacing)
                  // record the total duration for HUD progress
                  try { player._shieldDuration = 15000; } catch(e) { player._shieldDuration = 15000; }
                  // grant a very short invulnerability to avoid immediate follow-up hits after pickup
                  try { player.invulnerableUntil = Date.now() + 2000; } catch(e) {}
                  try { player._shieldWarned = false; } catch(e){};
              ...
              try {
                ...
                player.mulchUntil = now + 20000; // 20 seconds
                try { if (player) player._mulchDuration = 20000; } catch(e) {}
            ...
            try {
              ...
              try {
                ...
                if (timer) {
                  ...
                  else if (now < (player.mulchUntil || 0)) { remaining = Math.max(0, (player.mulchUntil || 0) - now); total = player._mulchDuration || 20000; }
  ...
  function draw() {
    ...
    try {
      if (Date.now() < (player.shieldUntil || 0)) {
        ...
        const _total = (player && player._shieldDuration) ? player._shieldDuration : 15000;
    ...
    if (versionEl) {
      try {
        versionEl.textContent = 'v' + version + ' — High: ' + displayHigh + (autoPauseEnabled ? ' — Auto-pause: On' : ' — Auto-pause: Off') + ' • Level: ' + (typeof levelNumber !== 'undefined' ? levelNumber : 1);
      } catch (e) { /* ignore DOM errors */ }
    }
    ...
    try {
      ...
      if (Date.now() < (canvasHitFlashUntil || 0) && !prefersReducedMotion) {
        const remaining = (canvasHitFlashUntil || 0) - Date.now();
        const alpha = Math.max(0, Math.min(1, remaining / 320)); // slightly longer fade for clearer hit feedback
        ctx.save();
        ...
        grad.addColorStop(0, 'rgba(232,255,186,' + (1.00 * alpha).toFixed(3) + ')');
        grad.addColorStop(0.6, 'rgba(232,255,186,' + (0.66 * alpha).toFixed(3) + ')');
